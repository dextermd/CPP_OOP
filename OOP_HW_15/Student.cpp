...
Student::Student(const char* surname, const char* name, const int year, const char* phone, const char* education, const int group_num, ...)
{
...
ostream& operator<<(std::ostream& out, const Student& obj)
{
    out << "\nФамилия              : " << obj.Surname;
    out << "\nИмя                  : " << obj.Name;
    out << "\nГод рождения         : " << obj.YearOfBirth;
    out << "\nТелефон              : " << obj.Phone;
    out << "\nУчебное заведение    : " << obj.EducationName;
    out << "\nНомер группы         : " << obj.GroupNumber;
    out << "\nОценки               : ";
...
istream& operator>>(std::istream& in, Student& obj)
{
    ...
    obj.Surname = new char[strlen(buf) + 1];
    strcpy_s(obj.Surname, strlen(buf) + 1, buf);
    ...
    strcpy_s(obj.Name, strlen(buf) + 1, buf);
    ...
    cout << "\nВведите Телефон: ";
    in.getline(obj.Phone, sizeof(obj.Phone));

    cout << "\nВведите Название Учебного заведения: ";
    in.getline(buf, sizeof(buf));

    if (obj.EducationName)
        delete[] obj.EducationName;

    obj.EducationName = new char[strlen(buf) + 1];
    strcpy_s(obj.EducationName, strlen(buf) + 1, buf);
