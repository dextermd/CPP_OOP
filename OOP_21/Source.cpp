...
int main()
{
	...
	/*
		...
		Каждый элемент (узел) дерева имеет не более двух элементов следующих за ним (потомков) и не более одного предыдущего (родителя).
	*/

	/*
		Бинарное (двоичное) дерево поиска – это бинарное дерево, для которого выполняются следующие дополнительные условия
		(свойства дерева поиска):
			- оба поддерева – левое и правое, являются двоичными деревьями поиска;
			- у всех узлов левого поддерева произвольного узла X значения ключей данных меньше, чем значение ключа данных самого узла X;
			- у всех узлов правого поддерева произвольного узла X значения ключей данных не меньше, чем значение ключа данных узла X.
		Данные в каждом узле должны обладать ключами, для которых определена операция сравнения меньше.
	...
	*/
	...
	t.Show();
	cout << endl;
	cout << "\nTree Count = " << t.getCount() << endl;

	// Поиск значения в дереве
	auto start = chrono::high_resolution_clock::now();
	cout << t.Find_Element(17) << endl;
	auto end = chrono::high_resolution_clock::now();
	auto duration = chrono::duration_cast <chrono::microseconds>(end - start);
	cout << "Find_Element: " << duration.count() << " Time." << endl;

	auto start_r = chrono::high_resolution_clock::now();
	cout << t.R_Find_Element(17) << endl;
	auto end_r = chrono::high_resolution_clock::now();
	auto duration_r = chrono::duration_cast <chrono::microseconds>(end_r - start_r);
	cout << "R_Find_Element: " << duration_r.count() << " Time." << endl;
	...
	// -------------------------------------------------------------------------------------
	// Записать и продемонстрировать  для MyArray  и для Шаблона списка  ListTwo, ListOne
